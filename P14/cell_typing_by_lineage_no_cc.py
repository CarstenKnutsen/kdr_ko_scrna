...
leiden_ct_dict = {
    ...
    "Immune": {
        ...
        "7": "Monocyte",
        "8": "low-quality Alveolar macrophage",
        "9": "Dendritic cell",
        "10":  "doublet",
        "11": "Dendritic cell",
        "12":"Neutrophil",
...
if __name__ == "__main__":
    ...
    colors = adata.uns['Cell Subtype_colors'].copy()
    print(colors)
    ...
    for lineage in adata.obs['Lineage'].cat.categories:
        ...
        if lineage == 'Mesenchymal':
            ...
            sc.pl.dotplot(lin_adata, weird_cells, groupby=f"leiden_{lineage}_no_cc", show=False,
                          save='weird_cells')
        result = {group: values['Cell Subtype'][
            ~values['Cell Subtype'].str.startswith("Proliferating")].value_counts().idxmax()
    ...
    for ct in adata.obs['Cell Subtype_no_cc'].cat.categories:
        try:
            ...
            sc.pl.palettes.default_102[add + len(adata.obs['Cell Subtype'].cat.categories)]
            add == 1
