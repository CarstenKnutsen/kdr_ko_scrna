...
def subcluster_celltype(adata,celltype,output_fol):
    ...
    lin_adata = adata[adata.obs['Cell Subtype_no_cc']==celltype]
    try:
    ...
    sce.pp.harmony_integrate(lin_adata, key='Library', max_iter_harmony=20)
    ...
    for color in [f'leiden_{celltype}','Library','Treatment','Genotype','Cell Subtype','Cell Subtype_no_cc','doublet_score',f'leiden_{lineage}','log1p_total_umis','log1p_n_genes_by_umis','ambient_score','doublet_score','proliferation_score']:
    ...
    plot_obs_abundance(lin_adata,f"leiden_{celltype}",hue="Library",ordered=True,as_percentage=True,save=f'{output_ct}/{celltype}_leiden.png')
    try:
...
## Subcluster each cell type
    for celltype in adata.obs['Cell Subtype_no_cc'].unique():
        subcluster_celltype(adata,celltype,f"{output}/subcluster_w_harmony")