'''Goal:Trajectory inference for kdr ko p14
Date:250205
...
if __name__ == '__main__':
    ...
    for comp,comp_dict in comps.items():
        ...
        for lib in adata_cts.obs['Library'].cat.categories:
            figures_lib= f'{figures_comp}/{lib}'
            os.makedirs(figures_lib, exist_ok=True)
            lib_adata = adata_cts[adata_cts.obs['Library']==lib]
            # lib_adata.X = lib_adata.layers['log1p'].copy()
            run_velocity_routine(lib_adata, adata_velocity, celltype, root_ct,terminal_cts, figures_lib,f'{data}/{adata_name}_{comp}_velocity_{lib}.gz.h5ad', umap=True)
