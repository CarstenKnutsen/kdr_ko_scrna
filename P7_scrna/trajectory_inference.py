'''Goal:Trajectory inference for kdr ko p7
Date:250205
...
comps = {
    ...
    #              'terminal_cts':['Cap2']
    #              },
    'alveolar_epithelial': {'cts': ['AT1', 'AT1_AT2', 'AT2'],
                    'root_ct': 'AT2',
                    'terminal_cts': ['AT1']
                    },
...
def run_velocity_routine(adata_v_trim,celltype,root_ct,terminal_cts,figure_dir,output_file):
...
if __name__ == '__main__':
    for comp,comp_dict in comps.items():
        adata_all = sc.read(f'{data}/{adata_name}_celltyped.gz.h5ad')
        ...
        adata_cts = adata_all[adata_all.obs[celltype].isin(cts)].copy()
        if comp=='capillaries':
            new_colors = []
        ...
        sc.settings.figdir = figures_comp
        scv.settings.figdir = figures_comp
        sc.pp.highly_variable_genes(adata_cts, batch_key='Library')
        sc.pp.pca(adata_cts)
        sc.pp.neighbors(adata_cts, use_rep="X_pca")
        sc.tl.umap(adata_cts, min_dist=0.5)
        sc.pl.umap(adata_cts, color=['Library', 'Treatment', celltype], save='_metadata')
        adata_v_trim = adata_velocity[adata_cts.obs_names, :].copy()
        adata_v_trim = scv.utils.merge(adata_cts, adata_v_trim)
        adata_v_trim.X = adata_v_trim.layers['raw'].copy()
        del adata_all
        del adata_velocity
        # run_velocity_routine(adata_v_trim,celltype,root_ct,terminal_cts,figures_comp,f'{data}/{adata_name}_{comp}_velocity.gz.h5ad')
        for lib in adata_cts.obs['Treatment'].cat.categories:
            figures_lib= f'{figures_comp}/{lib}'
            os.makedirs(figures_lib, exist_ok=True)
            lib_adata = adata_v_trim[adata_v_trim.obs['Treatment']==lib]
            print(lib)
            print(lib_adata)
            sc.settings.figdir = figures_lib
            scv.settings.figdir = figures_lib
            lib_adata.X = lib_adata.layers['log1p'].copy()
            sc.pp.highly_variable_genes(lib_adata)
            sc.pp.pca(lib_adata)
            sc.pp.neighbors(lib_adata, use_rep="X_pca")
            sc.tl.umap(lib_adata, min_dist=1)
            sc.pl.umap(lib_adata, color=['Library', 'Treatment', celltype], save='_metadata')
            lib_adata.X = lib_adata.layers['raw'].copy()
            run_velocity_routine(lib_adata, celltype, root_ct, terminal_cts, figures_lib,f'{data}/{adata_name}_{comp}_velocity.gz.h5ad')
