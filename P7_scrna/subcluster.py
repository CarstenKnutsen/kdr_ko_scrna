...
output = "data/figures/subcluster"
...
def subcluster_celltype(adata,celltype,output_fol):
    ...
    for color in [f'leiden_{celltype}','Library','Treatment','Genotype','Cell Subtype','doublet_score',f'leiden_{lineage}','log1p_total_umis','log1p_n_genes_by_umis','ambient_score','doublet_score',]:
...
## Subcluster each cell type
    for celltype in adata[adata.obs['Lineage']=='Mesenchymal'].obs['Cell Subtype'].unique():
